...
#---------------------------------------------------------
set CLK_PER 12
# squeeze clock add clock before this
create_clock -name $clkname -period $CLK_PER -waveform "0 [expr $CLK_PER / 2]" $clkname
set_clock_uncertainty $CLK_SKEW $clkname
# set area constraint
set_max_area 0

# and pipeline design and print result
optimize_registers > optim_reg.log
#compile -incr -map_effort medium > compile_${CLK_PER}ns.log
report_qor > qor_pipe_${CLK_PER}ns.rpt
report_timing > timing_pipe_${CLK_PER}ns.rpt
...
set CLK_PER 10
#create clock of the new clock period 
create_clock -name $clkname -period $CLK_PER -waveform "0 [expr $CLK_PER / 2]" $clkname
set_clock_uncertainty $CLK_SKEW $clkname
...
# compile incr with the squeezed clock and print the timinig report
compile -incr -map_effort medium > compile_${CLK_PER}ns.log
report_qor > qor_pipe_${CLK_PER}ns.rpt
report_timing > timing_pipe_${CLK_PER}ns.rpt
